...
Some SymCC targets (alwaysMatches, alwaysAllows, alwaysDenies, etc.) can cause cvc5 to hang on complex SMT queries generated by the fuzzer. This is a fundamental limitation of SMT-based fuzzing. The `FuzzSymCCNeverErrors` target is stable and recommended. Use `make fuzz-symcc-all` which runs only stable targets.
